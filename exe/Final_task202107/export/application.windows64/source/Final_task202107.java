...
public class Final_task202107 extends PApplet {
...
public void draw_maingameSetup(int route) {
...
public void draw_game() {
  if ("選択肢".equals(name[idx])) {
    ...
    background(0);
    fill(0, 0, 99);
    draw_msg(msg[idx], PApplet.parseInt(width-textWidth(msg[idx]))/2, height/2, width-25, height, 30);
    ...
    if (PApplet.parseInt(msg[idx])==1) {
      draw_miniGame1();
    } else if (PApplet.parseInt(msg[idx])==2) {
      draw_miniGame2();
    } else if (PApplet.parseInt(msg[idx])==3) {
      draw_miniGame3();
    }
    ...
    if (Score<=PApplet.parseInt(Criteria[0])) {
      draw_routeChange(0, msg[idx+1], type);
    } else {
      if (Criteria.length>1) {
        for (int  i=1; i  < Criteria.length; i++) {
          if (PApplet.parseInt(Criteria[i-1])<Score&&Score<=PApplet.parseInt(Criteria[i])) {
            draw_routeChange(i, msg[idx+1], type);
          }
        }
        if (PApplet.parseInt(Criteria[Criteria.length-1])<Score) {
...
public void draw_msg(String m, int x, int y, int w, int h, int F) {
  selecting=false;
  textSize(F);
  textAlign(LEFT, TOP);
  float s = 10-constrain(m.length()/10, 2, 6);
  int i = PApplet.parseInt((millis()-startTime)/(s*10));
...
public void draw_stage(int alpa) {
  imageMode(CORNER);
  tint(0, 0, 99, alpa);
  image(stage, 0, 0, width, height);
}

public void draw_select(String m, int sx, int y, int F) {
  selecting=true;
  Displaying=false;
...
public void draw_routeChange(int i, String m, String type) {
...
public void draw_Characters(int S) {
  ...
  if ((millis()-startTime <= 0.5f*1000)&&(Displaying==true)) {
    for (int i=0; i<chara.length; i++) {
      String[] c = split(chara[i], "_");
      if (c[0].equals(TalkingChara)) {
        y[i]=-sin(radians(map(millis()-startTime, 0, 0.5f*1000, 0, 720)))*5;
      }
    }
  }
...
public void draw_charaChange(String m) {
  chara  =  split(m, "/");
...
public void draw_musicChange(String m) {
  music.close();
...
public void draw_endingSetup(String m) {
  music.close();
  music = minim.loadFile("sound/"+m+".mp3");
...
public void draw_ending(String[] m) {
  fill(0, 0, 99);
  textSize(30);
  textAlign(LEFT, TOP);
  float h = textAscent() + textDescent() + 10;
  music.play();
  if (millis()-startTime <= music.length()) {
    for (int i=0; i<6; i++) {
      int s = music.length()/6;
      if ((s*i < millis()-startTime) && (millis()-startTime <= s*(i+1))) {
        if ((s*i < millis()-startTime) && (millis()-startTime <= s*i+s/3)) {
          a = PApplet.parseInt(map(millis()-startTime, s*i, s*i+s/3, 0, 254));
        } else if ((s*i+s/3 < millis()-startTime) && (millis()-startTime <= s*i+s/3*2)) {
          a = 255;
        } else {
          a = 255-PApplet.parseInt(map(millis()-startTime, s*i+s/3*2, s*(i+1), 0, 255));
        }
        imageMode(CENTER);
    ...
    float y = map(millis()-startTime, 0, music.length(), height+10, -h*m.length);
...
public void draw_miniGame1() {
  ...
  for (int  i=0; i < imgReference.length; i++) {
    ...
    if ("drawer".equals(temp[0])) {
      drawer[PApplet.parseInt(temp[1])] = img[i];
    }
  }
  for (int i=2-1; 0<=i; i--) {
    for (int j=3-1; 0<=j; j--) {
...
public void draw_miniGame3() {
  ...
  if (mousePressed && mouseButton == RIGHT) {
    if (feedExist==0) {
      ...
      Fy=mouseY;
      for (int  i=0; i  < 3; i++) {
        v[i]=PApplet.parseInt(random(5, 30));
        c[i]=PApplet.parseInt(random(0, 359));
      }
      for (int  i=0; i  < y.length; i++) {
        y[i] = PApplet.parseInt(dist(0, 0, Fx, Fy)+F.height);
      }
    }
  }
    ...
    for (int  i=0; i  < 3; i++) {
      ...
      if (feedExist==0) {
        if (y[0]<0&&y[1]<0&&y[2]<0) {
          feedExist=-1;
          for (int  j=0; j  < 3; j++) {
            v[j]=PApplet.parseInt(random(8, 20));
            c[j]+=PApplet.parseInt(random(0, 40));
          }
        }
      }
      int tmp=PApplet.parseInt(dist(0, 0, Fx, Fy)+F.height);
  ...
  if (State==-1) {
    ...
    for (int  i=0; i  < Branch.length; i++) {
      ...
      if (Branch[i].indexOf(",")!=-1) {
        route[i]=PApplet.parseInt(temp[0]);
        ID[i]=PApplet.parseInt(temp[1]);
        Stage[i] = temp[2];
        Music[i] = temp[3];
        C[i] = temp[4];
        m[i]=temp[5];
        clear[i]=PApplet.parseInt(temp[6]);
      }
    }
    ...
    if (State>=1) {
      if (State==1) {
        draw_maingameSetup(PApplet.parseInt(route[selectNumber]));
...
public void draw_ReHome(boolean JustStarting) {
...
public void draw_BranchSaveInitialize(int save) {
  int[]clear = new int [Branch.length];
  for (int  i=0; i  < Branch.length; i++) {
    String[] temp = split(Branch[i], ",");
    clear[i]=PApplet.parseInt(temp[6]);
...
public void draw_InitializeButton() {
...
public void mouseWheel(MouseEvent move ) {
  if (gameState==1) {
...
public void stop() {
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "Final_task202107" };
    if (passedArgs != null) {
